...
export function renderDiscountCoupon(
  ...
  const heroRow = `<tr>
    <td style="background-color:${theme.primary};padding:56px 40px;text-align:center;">
      <p style="margin:0;font-size:12px;font-weight:${w.label};text-transform:uppercase;letter-spacing:2px;color:rgba(255,255,255,0.7);">Exclusive Offer</p>
      <h1 style="margin:12px 0 0;color:${theme.heroText};font-size:42px;font-weight:${w.hero};line-height:1.1;letter-spacing:-1px;">${esc(data.heroHeadline || `${data.discountAmount} Off`)}</h1>
      <div style="margin:28px auto 0;display:inline-block;background-color:rgba(255,255,255,0.12);border:2px dashed rgba(255,255,255,0.5);border-radius:8px;padding:14px 32px;">
        <p style="margin:0;font-size:11px;font-weight:${w.label};text-transform:uppercase;letter-spacing:2px;color:rgba(255,255,255,0.7);">Use Code</p>
  ...
  <tr>
    <td style="padding:36px 40px;background-color:${theme.bgLight};">
      <p style="margin:0 0 24px;font-size:13px;font-weight:${w.label};text-transform:uppercase;letter-spacing:1.5px;color:${theme.accent};text-align:center;">How to Redeem</p>
      ${validSteps
        .map(
  ...
  <tr>
    <td style="padding:36px 40px;text-align:center;">
      <p style="margin:0 0 20px;font-size:16px;color:${theme.text};font-weight:${w.closingLine};">Don&rsquo;t let this offer slip away.</p>
      ${ctaButton(data.ctaText, data.ctaUrl, theme, font, w.cta)}
      <p style="margin:16px 0 0;font-size:13px;color:${theme.textMuted};">Use code <strong style="font-weight:${w.label};">${esc(data.couponCode)}</strong> at checkout.</p>
    </td>
  </tr>`;
