...
export function renderPromotional(
  ...
  const heroRow = `<tr>
    <td style="background-color:${theme.primary};padding:56px 40px;text-align:center;">
      <h1 style="margin:0;color:${theme.heroText};font-size:36px;font-weight:${w.hero};line-height:1.15;letter-spacing:-0.5px;">${esc(data.heroHeadline)}</h1>
      ${
        data.supportingLine
  ...
  const offerRow = data.offerDescription
    ...
    <td style="padding:36px 40px;background-color:${theme.bgLight};text-align:center;">
      <p style="margin:0;font-size:14px;font-weight:${w.label};text-transform:uppercase;letter-spacing:1px;color:${theme.accent};">The Offer</p>
