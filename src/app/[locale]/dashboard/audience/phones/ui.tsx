...
export function PhonesAudienceList({ initialAudiences, locale }: { initialAudiences: Row[]; locale: string }) {
  ...
  async function createAudience() {
    ...
    try {
      const res = await fetch("/api/audiences", {
        ...
        body: JSON.stringify({ name: newName.trim(), audience_type: "phone" }),
      });
      ...
      router.push(`/dashboard/audience/phones/${json.audience.id}`);
    } catch (e) {
    ...
    <>
            ...
            <ul className="divide-y divide-zinc-100">
              {audiences.map((a) => (
                <li key={a.id} className="flex items-center justify-between gap-4 px-5 py-4">
                  ...
                  <button
                    type="button"
                    onClick={() => router.push(`/dashboard/audience/phones/${a.id}`)}
      ...
      {showCreate ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="create-phone-audience-title"
          onMouseDown={(e) => {
        ...
        >
          <div className="w-full max-w-md rounded-xl border border-zinc-200 bg-white p-5 shadow-xl">
            <h2 id="create-phone-audience-title" className="text-base font-semibold text-ink">
              {t("createAudience")}
            </h2>
