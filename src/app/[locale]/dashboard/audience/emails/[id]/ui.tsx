...
export function EmailAudienceDetail({
  audienceId,
  initialAudience,
}: {
  audienceId: string;
  ...
  async function addEmails() {
    setError(null);
    setMessage(null);
    const emails = parseEmailInput(bulkText);
    if (!emails.length) {
    ...
    try {
      const res = await fetch(`/api/audiences/${audienceId}/members`, {
        ...
        body: JSON.stringify({ values: emails }),
      });
      ...
      setBulkText("");
      setShowAdd(false);
      setMessage(t("emailsAdded", { added: emails.length, total: count }));
      setPage(1);
      await loadMembers();
    } catch (e) {
  ...
  async function deleteList() {
    ...
    try {
      ...
      router.push("/dashboard/audience/emails");
    } catch (e) {
  ...
  async function deleteSelected() {
    ...
    try {
      ...
      setMessage(t("emailsRemoved", { count: json.deleted ?? selected.size }));
      ...
      <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm">
        {loading ? (
          ...
          <div className="px-5 py-12 text-center">
            <p className="text-sm text-ink-muted">{t("noEmailsYet")}</p>
            <button
              type="button"
              onClick={() => setShowAdd(true)}
