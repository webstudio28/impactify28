...
export async function GET(req: Request, ctx: Ctx) {
  ...
  const { data: campaign, error: cErr } = await supabase
    .from("campaigns")
    .select("id, user_id, email_html, channel")
    .eq("id", id)
    .single();
  ...
  const htmlRaw = typeof campaign.email_html === "string" ? campaign.email_html : "";
  if (!htmlRaw.trim()) {
    return NextResponse.json({ error: "No email HTML yet" }, { status: 400 });
  }
