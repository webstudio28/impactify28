...
export function PhonesAudienceClient({ initialAudiences }: { initialAudiences: Row[] }) {
  ...
  async function addNumbers() {
    ...
    const lines = bulkText
      ...
      .filter(Boolean);
    if (!lines.length) {
      setError("Paste one phone number per line");
      return;
    }
    setBusy(true);
    try {
  ...
  return (
    <div className="space-y-8">
      <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
        <h2 className="text-sm font-semibold text-ink">New phone list</h2>
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
        <h2 className="text-sm font-semibold text-ink">Add numbers</h2>
        <p className="mt-1 text-xs text-ink-muted">Paste CSV lines or one number per row.</p>
